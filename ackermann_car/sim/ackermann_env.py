...
class _KinematicBicycleSim:
    ...
    def step(self, velocity_cmd: float, steering_angle: float,
             disturbance: tuple = (0.0, 0.0, 0.0)) -> None:
        self.x, self.y, self.yaw, self.v = _bicycle_step(
            self.x, self.y, self.yaw, self.v,
            velocity_cmd, steering_angle, self.DT, disturbance,
        )
        self.t += self.DT

    def get_state(self) -> Dict[str, float]:
        return {"x": self.x, "y": self.y, "yaw": self.yaw,
                "velocity": self.v, "timestamp": self.t}


def _bicycle_step(
    x: float, y: float, yaw: float, v: float,
    velocity_cmd: float, steering_angle: float,
    dt: float,
