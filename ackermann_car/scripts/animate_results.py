...
if __name__ == "__main__":
    ...
    ax_road.plot(
        df["x"], df["y"], color="cyan", lw=0.6, alpha=0.35, zorder=2, label="Full path"
    )
    ...
    init_pts = _car_polygon(
        df_anim["x"].iloc[0], df_anim["y"].iloc[0], df_anim["yaw"].iloc[0]
    )
    car_poly = patches.Polygon(
        init_pts, closed=True, fc="tomato", ec="darkred", lw=1.2, zorder=6
    )
    ...
    def _update(frame_idx: int):
        ...
        arr_len = v * 0.5
        vel_arrow.xy = (x + arr_len * math.cos(yaw), y + arr_len * math.sin(yaw))
