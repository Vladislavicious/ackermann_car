...
from examples.quadrotor_drone.sim.road import Road
from examples.quadrotor_drone.sim.corridor_limits import CorridorLimits

OUTPUT_PATH = _REPO_ROOT / "examples" / "quadrotor_drone" / "sim" / "mujoco_ackermann_generated.xml"

# Geometry parameters
SEG_HALF_LEN  = 0.22    # half-length of each road stripe box [m]
SEG_HALF_W    = 0.04    # half-width of stripe [m]
SEG_HALF_H    = 0.001   # very thin
...
def _box_geom(name: str, x: float, y: float, heading: float,
    ...
    """Return a single <geom> XML line for a rotated flat box."""
    # MuJoCo euler angles: roll, pitch, yaw (radians or degrees with angle="radian")
...
def build_xml(road: Road, corridor: CorridorLimits) -> str:
    ...
    for i, (wx, wy, wh) in enumerate(waypoints[::SEG_STEP]):
        ...
        # Centre line — yellow dashes (every other segment)
        if (i % 2) == 0:
    ...
    xml = f"""<!--
  mujoco_ackermann_generated.xml
  ================================
  Auto-generated by generate_mujoco_road_xml.py
  ...
  2D DOF (Assignment §2):
    joint_x, joint_y, joint_yaw only — z/roll/pitch have no joints → frozen.
-->
<mujoco model="ackermann_car_scurve">
  ...
  <worldbody>
    ...
    <!-- Ground plane (grey road surface) -->
...
{right_xml}
    ...
    <!-- ══════════════════════════════════════════════════════════
         Ackermann car
         3-DOF planar constraint:
           joint_x   → x translation
           joint_y   → y translation
           joint_yaw → yaw rotation
         z, roll, pitch have NO joints → MuJoCo keeps them fixed.
         ══════════════════════════════════════════════════════════ -->
  ...
  <!-- Actuators kept for completeness (not used in kinematic backend) -->
  <actuator>
...
def main() -> None:
    print("[generate_mujoco_road_xml] Building road …")
    road     = Road()
    ...
    print(f"[generate_mujoco_road_xml] Saved → {OUTPUT_PATH.relative_to(_REPO_ROOT)}")

    # Quick validation: try loading with MuJoCo if available
    try:
        import mujoco
        m = mujoco.MjModel.from_xml_path(str(OUTPUT_PATH))
        print(f"[generate_mujoco_road_xml] MuJoCo validation OK  "
              f"(nbody={m.nbody}, ngeom={m.ngeom})")
    except ImportError:
        print("[generate_mujoco_road_xml] MuJoCo not installed — skipping validation")
    except Exception as e:
        print(f"[generate_mujoco_road_xml] MuJoCo validation FAILED: {e}")
        raise
